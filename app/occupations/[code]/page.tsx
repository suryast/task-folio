...
async function getOccupation(code: string): Promise<Occupation> {
  const API_BASE = process.env.NEXT_PUBLIC_API_URL || 'https://taskfolio-au-api.hello-bb8.workers.dev'
...
async function getTasks(code: string): Promise<{ tasks: Task[] }> {
  const API_BASE = process.env.NEXT_PUBLIC_API_URL || 'https://taskfolio-au-api.hello-bb8.workers.dev'
  const res = await fetch(`${API_BASE}/api/tasks/${code}`, {
    next: { revalidate: 3600 } // Cache for 1 hour
  })
...
export default async function OccupationPage({
  ...
  try {
    const [occupation, tasksData] = await Promise.all([
      getOccupation(code),
      getTasks(code)
    ])
