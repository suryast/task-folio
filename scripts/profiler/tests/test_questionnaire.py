...
def test_get_tasks_for_occupation():
    tasks = get_tasks_for_occupation("2613")  # Software and Applications Programmers
    assert len(tasks) > 0
...
def test_build_profile_basic():
    ...
    selections = {
        tasks[0]["task_description"]: {"does_task": True, "time_pct": 50},
        tasks[1]["task_description"]: {"does_task": True, "time_pct": 30},
        tasks[2]["task_description"]: {"does_task": True, "time_pct": 20},
    }
    profile = build_profile("2613", selections)
    assert str(profile["anzsco_code"]) == "2613"
...
def test_calculate_personalised_score():
    tasks = get_tasks_for_occupation("2613")
    selections = {
        tasks[0]["task_description"]: {"does_task": True, "time_pct": 100},
    }
    profile = build_profile("2613", selections)
...
def test_score_differs_by_task_selection():
    ...
    tasks = get_tasks_for_occupation("2613")
    sel_a = {tasks[0]["task_description"]: {"does_task": True, "time_pct": 100}}
    sel_b = {tasks[-1]["task_description"]: {"does_task": True, "time_pct": 100}}
